...
from .models import Senai, Sala, Usuario
...
def homepageAdmin(request):
    ...
    if request.method == 'POST':
        if 'delete' in request.POST:
            ...
            all_booked = Sala.all_hours_booked()
            Sala.objects.filter(id__in=ids_to_delete).delete()
...
def cadastroUsuario(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            var_quartos = form.cleaned_data['quartos']

            usuario = Usuario.objects.create(nome=var_nome, sobrenome=var_sobrenome, email=var_email, idade=var_idade, quartos=var_quartos)
